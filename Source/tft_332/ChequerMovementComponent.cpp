...
FRotationTarget::FRotationTarget(UBoardNodeFinal *InNode, const float InAcceptableDeviation) :bActive(false), RotationTargetChequer(nullptr),
...
{
	UBoardNodeFinal *NextPathNode;  // 要去往的下一个格点
...
// 设置目标点
void UChequerMovementComponent::SetTargetNode(UBoardNodeFinal *InNode)
{
	// 取消对原目标点的标记
	if (TargetNode)
		TargetNode->CancelReservation(GetOwner());
...
// 设置当前棋格
void UChequerMovementComponent::SetCurrentNode(UBoardNodeFinal *InNode)
{
	// 逐格移动，进入新格时重置目标棋格
	if (TargetNode)
...
// 设置移动路径
void UChequerMovementComponent::SetChequerMovePath(TArray<UBoardNodeFinal *> InPath, bool bReverse)
{
	if (bReverse)
		Algo::Reverse(InPath);
...
void UChequerMovementComponent::CorrectCurrentNode()
{
	...
	if (!ChequerMovePath.Num())  // 已达终点
		return;
	UBoardNodeFinal *NextPathNode = ChequerMovePath.Top();
	if (!NextPathNode)
	{
		ChequerMovePath.Pop();
		return;
