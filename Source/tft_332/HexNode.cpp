...
//获取相邻棋格
TArray<UBoardNode*> UHexNode::GetNeighbors()
{
	TArray<UBoardNode*> TemNodes;
	if (RightUpNode)
		TemNodes.Add(RightUpNode);
	if (RightNode)
		TemNodes.Add(RightNode);
	if (RightDownNode)
		TemNodes.Add(RightDownNode);
	if (LeftDownNode)
		TemNodes.Add(LeftDownNode);
	if (LeftNode)
		TemNodes.Add(LeftNode);
	if (LeftUpNode)
		TemNodes.Add(LeftUpNode);
	return TemNodes;
}
	...
	FVector InOffset)
{
	...
	HexVerts[0] = Location - InOffset;//中心点
	...
	//UV
	TArray<FVector2D> UV;
	UV.Init(FVector2D(0.5f, 0.5f), 7);//初始的偏移量是（0.5，0.5）；
